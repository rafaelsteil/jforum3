...
<xsl:template match="testsuite" mode="class.details">
    ...
    <html>
       ...
       <script type="text/javascript" language="JavaScript"><![CDATA[
        ...
        function selectRows(name) {
        	var table = document.getElementById('methods');
        	var trs = table.tBodies[0].getElementsByTagName('tr');
        	for(var i = trs.length -1; i >= 0; i--) {
        		var t = trs[i].getElementsByTagName('td')[0].textContent;
        		if (t != name)
		...
		<xsl:otherwise>
            ...
            <table id="methods" class="details" border="0" cellpadding="5" cellspacing="2" width="95%">
		<xsl:call-template name="testcase.test.header"/>
		<tbody>
		<!--
                test can even not be started at all (failure to load the class)
...
<xsl:template match="testcase" mode="print.test">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
	    	<td><a href="#" onclick="selectRows(this.innerHTML)">
	    		<xsl:value-of select="substring-after(@classname, 'br.com.caelum.caelumweb2.integracao.')"/>
	    		</a>
	    	</td>
		</xsl:otherwise>
	</xsl:choose>
