 ...
 * @author Rafael Steil
 */
public class SearchActionsTestCase {
	...
	private SearchRepository searchRepository = context.mock(SearchRepository.class);
	private UserSession userSession = context.mock(UserSession.class);
	private RoleManager roleManager = context.mock(RoleManager.class);
	private ViewService viewService = context.mock(ViewService.class);
	private SearchActions action = new SearchActions(propertyBag, categoryRepository, config,
		searchRepository, userSession, viewService);

	@Test
	public void executeWithEmptyQueryShouldRedirectToFilters() {
		context.checking(new Expectations() {{
			one(viewService).redirectToAction(Actions.FILTERS);
		}});
	...
	public void executeFindThreeRecordsOneIsNotAllowedShouldRemoveExpectTwoRecords() throws Exception {
		final SearchResult result = new SearchResult(new ArrayList<Post>(Arrays.asList(
			this.newPost(1, 1), this.newPost(2, 1), this.newPost(3, 2))), 3);
		final SearchParams params = new SearchParams();
		params.setQuery("abc");

		context.checking(new Expectations() {{
			allowing(config).getInt(ConfigKeys.TOPICS_PER_PAGE); will(returnValue(30));
			allowing(userSession).getRoleManager(); will(returnValue(roleManager));
			one(searchRepository).search(params); will(returnValue(result));
			allowing(roleManager).isForumAllowed(1); will(returnValue(true));
			allowing(roleManager).isForumAllowed(2); will(returnValue(false));
			one(categoryRepository).getAllCategories(); will(returnValue(new ArrayList<Category>()));

			one(propertyBag).put("results", result.getResults());
			one(propertyBag).put("searchParams", params);
			one(propertyBag).put("pagination", new Pagination(config, 0));
		...
		Post post = new Post(); post.setId(3);
	...
	private Post newPost(int postId, int forumId) {
		...
		post.setForum(new Forum()); post.getForum().setId(forumId);
		return post;
	}
}
